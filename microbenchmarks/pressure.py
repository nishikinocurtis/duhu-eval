...
put_futs = [[] for _ in range(len(node_ids))]

for i in range(NUM_TRIALS * len(node_ids)):
    put_fut = force_put_one_array.options(
        scheduling_strategy=ray.util.scheduling_strategies.NodeAffinitySchedulingStrategy(
            node_id=node_ids[i % len(node_ids)]['NodeID'],
            soft=False)
    ).remote()
    put_futs[i % len(node_ids)].append(put_fut)

for i in range(len(node_ids)):
    put_futs[i] = ray.get(put_futs[i]) # group of handles

start_time_after_seal = time.time()

handles = []

# force node 2 3 4 to get data from 1
for i in range(NUM_WORKERS):
    worker_refs = put_futs[0]
